...
docker exec -i sql_server_container /opt/mssql-tools18/bin/sqlcmd -S localhost -U sa -P "<SA_PASSWORD>" -C -i /dev/stdin < database/02_create_app_user.sql
...
- No backend, prefira usar o login da aplicação (ex.: `access_app`) em vez de `sa`.
